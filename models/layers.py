...
### Added pre_trained layer
class Pre_trained(Layer):
    def __init__(self, units, w, b):
        super().__init__()
        w0 = tf.constant_initializer(w)
        b0 = tf.constant_initializer(b)
        self.w = tf.Variable(initial_value=w0(shape = (697, units), dtype = tf.float32), trainable = False)
        self.b = tf.Variable(initial_value=b0(shape = (units, ), dtype = tf.float32), trainable = False)
        
    def call(self, x, lab):
        lab = tf.matmul(lab, self.w) + self.b
